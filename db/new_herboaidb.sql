...
CREATE VIRTUAL TABLE disease_vec USING vec0(
...
CREATE VIRTUAL TABLE plant_vec USING vec0(
...
CREATE VIRTUAL TABLE prep_vec USING vec0(
...
CREATE TABLE knowledge_chunks (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  entity_type TEXT NOT NULL,       -- 'plant', 'disease', 'preparation', 'general'
  entity_id INTEGER,               -- NULL for 'general' chunks
  section TEXT,                    -- 'ayurvedic_uses', 'classical_ref', 'modern_research', etc.
  content TEXT NOT NULL,           -- 2–6 sentence paragraph
  source TEXT,                     -- 'Charaka Samhita', 'Journal XYZ 2018', 'Internal notes'
  embedding BLOB                   -- float32[384] from Python (no vec0)
);
