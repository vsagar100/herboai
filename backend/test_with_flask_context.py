#!/usr/bin/env python3
"""
Test the full query pipeline with Marathi using Flask context.
"""
import sys
import os
import json

sys.path.insert(0, str(os.path.dirname(os.path.abspath(__file__))))

# Import Flask app
from run import create_app

app = create_app()

test_query = "मधुमेह साठी काय घ्यावं?"  # "What to take for diabetes?"

print("=" * 80)
print(f"Testing Full Query Pipeline with Flask Context")
print("=" * 80)
print(f"Query: {test_query}")
print()

with app.app_context():
    from services.chat import handle_chat
    
    try:
        result = handle_chat(test_query, lang="mr", session_id=None)
        
        print("RESULT:")
        print(json.dumps(result, ensure_ascii=False, indent=2))
        
        if "error" in result:
            print(f"\n❌ ERROR: {result['error']}")
        elif "structured" in result:
            prep_count = len(result.get("structured", {}).get("preparations", []))
            print(f"\n✅ SUCCESS: Got {prep_count} preparations")
        else:
            print("\n⚠️  Unexpected response structure")
            
    except Exception as e:
        print(f"❌ EXCEPTION: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()

print("=" * 80)
