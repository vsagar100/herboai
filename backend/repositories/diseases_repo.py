...
def get_disease(disease_id: int, lang: str = "en"):
    lang = normalize_lang(lang)
    db = get_db()
    cur = db.cursor()
    ...
    if row:
        ...
        result["localized_name"] = get_localized_field("disease", disease_id, "name", lang)
        result["localized_description"] = get_localized_field("disease", disease_id, "description", lang)
        result["localized_symptoms"] = get_localized_field("disease", disease_id, "symptoms", lang)
        result["localized_causes"] = get_localized_field("disease", disease_id, "causes", lang)
        result["localized_prevention_tips"] = get_localized_field("disease", disease_id, "prevention_tips", lang)
        return result
    return None
